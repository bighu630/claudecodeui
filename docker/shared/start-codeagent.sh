...
# Auto-start CodeAgent server in background if not already running.
# This script is sourced from ~/.bashrc on sandbox shell open.

if ! pgrep -f "server/index.js" > /dev/null 2>&1; then
  nohup codeagent start --port 3001 > /tmp/codeagent-ui.log 2>&1 &
  disown

  echo ""
  echo "  CodeAgent is starting on port 3001..."
  echo ""
