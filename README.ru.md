...
---
## Система ролевых сессий

CodeAgent UI использует систему ролевых сессий для управления сложными рабочими процессами агентов. Каждый проект при создании получает два корневых дерева сессий, которые следуют строгой иерархии наследования «родитель-потомок».

### Структура дерева сессий

Каждый проект инициализируется с двумя независимыми корневыми сессиями:

- **tech_lead** — отвечает за архитектурные решения, анализ осуществимости и декомпозицию требований. Передаёт задачи реализации сессиям feature_lead.
- **ops** — управляет развёртыванием, настройкой окружения и поддержкой инфраструктуры. Этот корень независим и не создаёт дочерних сессий.

Допустимые производные отношения:

- `tech_lead → feature_lead` — сессия tech_lead может создавать дочерние сессии feature_lead
- `feature_lead → worker` — сессия feature_lead может создавать дочерние сессии worker
- `worker → ✗` — worker предназначены только для выполнения и не могут порождать новые сессии
- `ops → ✗` — сессии ops работают независимо

### Обязанности ролей

| Роль | Обязанности |
|---|---|
| **tech_lead** | Архитектурные решения, оценка осуществимости, анализ требований. Передаёт задачи feature_lead. |
| **feature_lead** | Планирование на уровне кода, декомпозиция реализации, координация worker. |
| **worker** | Строго следует спецификации задачи. Единственная ответственность. |
| **ops** | Развёртывание, окружение, поддержка инфраструктуры. |

### Стратегия пустого запуска

Дочерние сессии запускаются в пустом состоянии и получают только минимально необходимый контекст передачи (цель, ограничения, спецификацию задачи). История сессии **не наследуется** от родителя. Это предотвращает утечку ролей — worker действует только в рамках назначенной задачи, а не на основе всего разговора, который её породил.

### Разделение сессии и среды выполнения

Оркестраторная сессия — это **логическая идентичность** в дереве сессий, в то время как среда выполнения провайдера (сессия Claude, Codex, Cursor или Gemini) является **носителем выполнения**. Это разные понятия: одна оркестраторная сессия может быть привязана к одной сессии среды выполнения через `external_session_id`, но структура дерева, ролевые промпты и жизненный цикл управляются независимо от провайдера среды выполнения.
